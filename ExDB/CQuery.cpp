...
BOOL CQuery::Exec(LPCTSTR szSQL, BOOL bLog)
{
	...
	while (true)
	{
		...
		if (nCol > MAXCOL) {
			...
			cLog.AddTD("CQuery error :최대 컬럼 수를 초과했습니다");
			return FALSE;
		}
...
BOOL CQuery::SPCheck(char* ScriptName)
{
	CString qSql;
	...
	if (this->Exec((LPCTSTR)qSql) == TRUE)
	{
		result = (this->Fetch() != SQL_NO_DATA);
	}
...
BOOL CQuery::TableCheck(char* ScriptName)
{
	CString qSql;
