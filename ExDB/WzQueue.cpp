...
WZQueue::WZQueue(DWORD nMaxNode)
{
	m_Max_Node = nMaxNode;
...
BOOL WZQueue::AddToQueue(const LPBYTE pObject, DWORD nSize, BYTE headcode, int uindex)
{
	BOOL bRet = FALSE;
	EnterCriticalSection(&m_CriticalSection);
	...
	if (pNewNode)
	{
		LPBYTE pMsg = (LPBYTE)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, nSize);
...
ListNode* WZQueue::AddToQueueList(const LPBYTE pObject, DWORD nSize, BYTE headcode, int uindex)
{
	BOOL bRet = FALSE;
...
BOOL WZQueue::GetFromQueue(LPBYTE pObject, LPDWORD pSize, LPBYTE headcode, int* uindex)
{
	ListNode* pNode = NULL;
...
ListNode* WZQueue::GetCurData(LPBYTE pObject, LPDWORD pSize, LPBYTE headcode, int* uindex)
{
	if (m_pCur)
	{
		if (m_pCur->bSending == TRUE)
		{
			return FALSE;
		}
...
BOOL WZQueue::Pop(ListNode* pCur, LPBYTE pObject, int nOfs, int* nSize, int* sentbytes)
{
	BOOL bRet = FALSE;
